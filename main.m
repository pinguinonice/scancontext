...
parfor  n= 1:nGridX
    for m = 1:nGridY
        ...
        D{n,m} = scanContextDescriptor(ptCloud,'SensorOrigin',[X(n)        Y(m)],'RadialRange' ,maxRad);
...
rndPt=[X(10) Y(60) 0 ];
        rndD = scanContextDescriptor(ptCloud,'SensorOrigin',rndPt(1:2),'RadialRange' ,maxRad);
...
parfor n =1:nGridX
    for m =1:nGridY
    dist(n,m) = scanContextDistance(rndD,D{n,m} );
